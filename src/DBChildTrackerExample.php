    ...
    public function track(DBEvent $event) {
        switch ($event->type) {
            case DBEvent::AFTER_INSERT:
                error_log("A new object is inserted. It's id is ". $event->object->id);
                break;
            case DBEvent::AFTER_UPDATE:
                error_log("The object is updated. Modified fields are: ". implode(", ", $event->fields));
                break;
            ...
            case DBEvent::INTRAN_UPDATE:
                ...
                break;
            default:
                throw new \Exception('Этот трекер не умеет обрабатывать события типа '. DBEvent::typeName($event->type));
        }
    }
}
